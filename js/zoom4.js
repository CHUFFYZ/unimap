...
function fitCampusToScreen(campusName) {
    if (!map || !campuses[campusName]) return;

    const { w, h } = campuses[campusName];
    const bounds = [[0, 0], [h, w]];

    // maxBoundsViscosity impide mostrar fondo blanco alrededor del SVG
    map.setMaxBounds(bounds);

    // Ajusta la vista para que el SVG ocupe la pantalla actual sin importar la resolución
    map.fitBounds(bounds, { padding: [0, 0] });
}

function switchToCampus(campus) {
    currentCampus = campus;
    const { center, zoom } = campuses[campus];
    
    // Encuadra automáticamente según la resolución
    fitCampusToScreen(campus);
...
function flyToLocation(lat, lng, building, placeName, campus, isInterestPoint = false) {
    ...
    if (locationControls) locationControls.classList.remove('visible');
    ...
    const onMoveEnd = () => {
        if (isInterestPoint) {
            ...
            if (pinMarker) {
                ...
                // Desactivar autoPan a través de las opciones del popup del marker
                if (pinMarker.getPopup()) pinMarker.getPopup().options.autoPan = false;
                pinMarker.openPopup();
            ...
            const animateMarkerAndPopup = () => {
                if (targetMarker._icon) {
                    ...
                    // Desactivar autoPan en el popup existente antes de abrirlo
                    if (targetMarker.getPopup()) {
                        targetMarker.getPopup().options.autoPan = false;
                    }
    ...
    // Respaldo de seguridad para desbloquear isFlying si la animación se frena
    setTimeout(() => {
        if (isFlying) {
...
/*----------cerrar los popup al hacer click fuera ----------*/
document.addEventListener('click', function(event) {
    // si el panel no esta visible o el clic ocurrio dentro de el, no hacemos nada
    if (!detailsPanel.classList.contains('visible') || detailsPanel.contains(event.target)) {
        return;
    }

    // si el clic fue en un elemento interactivo del mapa como pines o marcadores, evitamos cerrar el panel por accidente
    if (event.target.closest('.leaflet-marker-icon, .leaflet-popup, .interest-point, .interest-pin')) {
        return;
    }

    // cerramos el panel al detectar un clic fuera de el
    detailsPanel.classList.remove('visible');
    
    // si hay un pin activo en el mapa, lo removemos
    if (currentPinMarker) {
        map.removeLayer(currentPinMarker);
        currentPinMarker = null;
    }
...
function showOSMLocationDetails(building, placeName, faculty, photos, comments, campus, showGoButton = false) {
    ...
    const commentsHTML = safeComments.map(comment => `<p>${comment}</p>`).join('');
    let derechosHTML = 'Algunas imagenes pudieron ser obtenidas de Google Maps';
    if (showGoButton) {
...
function createInterestPointMarker(lat, lng, title, building, photos, comments, campus) {
    ...
    icon: customIcon,
    zIndexOffset: 1000
}).bindPopup(`<b>Zona: ${title}</b><br><small>${building}</small>`, { autoPan: false }).addTo(map);
    marker.on('click', () => {
...
function toggleRouteDisplay(routeId) {
    ...
    // Si la ruta tocada ya estaba seleccionada, la desactivamos
    if (selectedRoutes.includes(routeId)) {
        clearRoutesAndStops();
    } else {
        // Limpiamos la ruta anterior para permitir solo UNA activa
        clearRoutesAndStops();
...
function updateRoutesMenu() {
    ...
    truckRoutes.forEach(route => {
        ...
        const iconUrl = route.icon || '/image/locations/CP/galeria/BUS/bus-icon.png'; // Fallback por si no se especifica icono
        
        menuHTML += `
            <div class="route-item ${isSelected ? 'active' : ''}" data-route-id="${route.id}">
                <img src="${iconUrl}" alt="${route.name}" class="route-icon" />
...
function createMarker(lat, lng, title, building, iconConfig, faculty, photos, comments, campus, isShared = false) {
    ...
    try {
        ...
        title: title,
        icon: customIcon
        }).bindPopup(`<b>${isShared ? 'Edificio' : 'Zona'}: ${title}</b><br><small>${faculty}</small>`, { autoPan: false });
        marker.on('click', () => {
