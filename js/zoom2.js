...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (osmMapElement) {
        ...
        // Mantener popups siempre abiertos
        Object.values(osmMap.campusMarkers).forEach(marker => {
            ...
            marker.on('click', () => {
                const campus = Object.keys(osmMap.campusMarkers).find(c => osmMap.campusMarkers[c] === marker);
    ...
    function switchToCampus(campus) {
        ...
        if (interestPointsActive[campus]) {
            toggleInterestPoints(campus);
            toggleInterestPoints(campus); // Reactivar puntos de interés si estaban activos
        }
    }
    ...
    function handleMapTransition(currentZoom, fromOSM = false) {
        ...
        if (currentZoom === minZoom && osmMap && osmMapElement && !fromOSM) {
            ...
            if (fromOSM) {
                ...
                const closestCampus = getClosestCampus();
                if (closestCampus) {
                    currentCampus = closestCampus;
                }
    ...
    if (osmMap) {
        osmMap.on('zoomend', () => {
            if (isOSMVisible) {
                ...
                if (osmZoom >= 18) {
                    handleMapTransition(osmZoom, true);
                }
            }
        });
    }
