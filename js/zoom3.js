...
document.addEventListener('DOMContentLoaded', function() {
    ...
    map = L.map('map', {
        ...
        zoomDelta: 0.3,
        zoomSnap: 0,
        zoomAnimation: false,         // Desactiva la interpolación frame por frame del SVG
    fadeAnimation: false,         // Desactiva el desvanecimiento entre capas
    markerZoomAnimation: false,   // Evita recalcular iconos durante el zoom
    preferCanvas: true            // Obliga a utilizar Canvas siempre que sea posible
    });
    ...
    const { center, zoom } = campuses['Campus Principal'];
    ...
    // Ajusta automáticamente el plano al contenedor de pantalla
    fitCampusToScreen('Campus Principal');
    ...
    function handleMapTransition(currentZoom, fromOSM = false) {
        ...
        // Evaluar la transición a OSM de forma independiente al tamaño de pantalla
        if (currentZoom <= -1.45 && osmMap && osmMapElement && !fromOSM) {
            if (!isOSMVisible) {
                isOSMVisible = true;
   ...
   window.addEventListener('resize', () => {
        if (map && !isOSMVisible) {
            fitCampusToScreen(currentCampus);
        }
    });
});